...
// ── Generar PDF usando el panel existente ─────────────────────────────────
function descargarPDF(nombrePaisaje) {
  const btn = document.querySelector('.pdf-btn');
  if (btn) { btn.textContent = '⏳ Generando...'; btn.disabled = true; }

  // Crear contenedor PDF con fondo blanco sobre el panel existente
  ...
  // Clonar el panel y aplicar estilos para PDF
  ...
  // Cambiar colores oscuros a claros en el clone
  ...
  logoDiv.style.cssText = 'display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #7a6e65;padding-bottom:12px;margin-bottom:16px;';
  logoDiv.innerHTML = `
    <img src="${LOGO_ARAUCO}" style="height:32px;object-fit:contain;" crossorigin="anonymous"/>
    <div style="text-align:right;font-family:Arial;">
      <div style="font-size:15px;font-weight:600;color:#1a1a1a;">${nombrePaisaje}</div>
      <div style="font-size:10px;color:#888;">${new Date().toLocaleDateString('es-CL', {day:'2-digit',month:'long',year:'numeric'})}</div>
    </div>
  `;
