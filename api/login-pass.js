...
export default async function handler(req, res) {
  ...
  if (action !== 'login') {
    ...
    if (action === 'change') {
      ...
      await redis(['DEL', `passtemp:${creds.email}`]); // el usuario ya eligió su clave → sin más alertas
    ...
    if (action === 'set') {
      ...
      await redis(['SET', `passtemp:${tgt}`, '1']); // clave asignada por admin → pedir cambio por seguridad
  ...
  if (creando) {
    ...
    await redis(['SET', `passtemp:${email}`, '1']); // primera clave → recordar cambiarla por seguridad
  ...
  const debeCambiar = !!(await redis(['GET', `passtemp:${email}`]));

  return res.status(200).json({ email: usuario.email, rol: usuario.rol, cargo: usuario.cargo || '', token, sessionId, debeCambiar });
}
